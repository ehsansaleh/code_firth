...
from utils.summary import get_csvh5files, read_csvh5
from utils.summary import beststat_summarizer, add_missing_dflt_vals
from utils.cfg import main_acc, reg_sources, summ_cond_vars
from utils.cfg import results_csv_dir, smry_tbls_dir, prop_cols, dfltvals_dict
from utils.cfg import csvdir_expname, expname_regcol
...
for _, (cond, df_cond) in enumerate(df_full.groupby(cond_variables)):
    ...
    df_collection = beststat_summarizer(df_cond.reset_index(drop=True), prop_cols=prop_cols,
                                        y_col=y_col, reg_column=reg_column,
                                        reg_sources=reg_sources, sync_rng=True, cond=cond)
